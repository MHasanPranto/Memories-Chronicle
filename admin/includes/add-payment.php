<?php 
  ...
  if (isset($_POST['confirm'])) {
    ...
    $to_be_paid = $_POST['to_be_paid'];

    $p_added_by = $_SESSION['admin-username'];
    $role = $_SESSION['role'];
    date_default_timezone_set('Asia/Dhaka');
    $payment_date = date("F j, Y, g:i a");

    $sql2 = "UPDATE events SET paid = '$paid', payment_status = 2 WHERE id = '$id'";
    if($conn->query($sql2) === TRUE) {

      $sql3 = "INSERT INTO `turnover`(`event_id`, `c_name`, `event_type`, `event_date`, `to_be_paid`, `paid`, `p_add_by`, `role`, `p_date`) 
                              VALUES ('$event_id','$c_name','$event_type','$event_date','$to_be_paid','$paid','$p_added_by','$role','$payment_date')";
        ...
        <main>
            ...
            <div class="table" style="top:50%; left:60%;">
                <table class="neumorphic" style="width:950px;">
                    <form action="./add-payment.php?id=<?php echo $_GET['id'] ?>" method="post">
                        <thead>
                          <tr>
                            <th colspan = "8"><h2>Add Client's Payment Amount </h2></th>
                          </tr>
                          <tr>
                        ...
                        <tbody>
                            ...
                            <tr>                         
                              ...
                              <input type="hidden" name="e_type" value = "<?php echo $row['event_type'] ?>">
                              <input type="hidden" name="e_date" value = "<?php echo $row['event_date'] ?>">
                              <input type="hidden" name="to_be_paid" value = "<?php echo $row['to_be_paid'] ?>">
                            ...
                            <tr>
                              <td colspan = '8'>
                                <a href='../payroll.php'><button class = "add-btn" type='button'>Cancle</button></a>
