  ...
  if ($error === 0) {
    if ($img_size > 1000000) {
      ...
      if (in_array($img_ex_lc, $allowed_exs)) {
        ...
        $sql = "INSERT INTO user(image,name,email,number,password) VALUE ('$new_img_name','$name','$email','$number','$password')";
        mysqli_query($conn, $sql);
        echo '<script>alert("Your have registered successfully")</script>';
        echo '<script type = "text/javascript"> window.location = "./login.php"</script>';
      }else {
